...
export default function MillerNewEditForm({ miller }) {
  ...
  const [disable,setDisable] = useState(miller ? true : false)
  ...
  const defaultValues = useMemo(
    () => ({
      name: miller?.name || '',
      milling_type:miller?.milling_type || '',
      contact_person: miller?.contact_person || '',
      phone_number: miller?.phone_number || '',
      email: miller?.email || '',
      pan_number: miller?.pan_number || '',
      gst_number: miller?.gst_number || '',
      type_of_firm: miller?.type_of_firm || '',
      address: miller?.address || '',
      state: miller?.state || '',
      branch: miller?.branch || '',
      district: miller?.district || '',
      procurement_area: miller?.procurement_area || '',
      pincode: miller?.pincode || '',
    }),
    [miller]
  );
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    //     .then((res) => {
    //       if (res?.data?.status == '201') {
    //         router.push(paths.dashboard.miller.distributor_list);
    //         enqueueSnackbar('Distributor added successfully');
    //       }
    //     });
    // } catch (err) {
      ...
      <Grid xs={12} md={8}>
        <Card>
          <Stack spacing={3} sx={{ p: 3 }}>
            ...
            >
              <RHFTextField  disabled={disable} name="name" label="Milling Unit Name" />
              <RHFAutocomplete
                name="milling_type"
                label="Milling Type"
